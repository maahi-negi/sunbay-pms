            ...
            <div class="row breadcrumbs-top">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{route('homeplans')}}">Homeplans</a>
                        </li>
                        <li class="breadcrumb-item">
                            <a href="{{route('home-design-types', ['elevation_id' => $elevation_id])}}">{{ ucwords($home_title) }}</a>
                        </li>
                        <li class="breadcrumb-item">
                ...
                @if(count($designs) >= 1)
                    @foreach($designs as $key => $design)
                        <tr id="row{{$design->id}}">
                            ...
                            <td class="text-dark">{{$design->product_id}}</td>
                            <td>
                                <div class="design-color" style="background: rgb({{$design->rgb_color}});height: 55px;overflow: hidden;">
                                    @if($design->thumbnail)
                                        <img class="img-fluid" src="{{asset('media/uploads/exterior/'.$design_type_slug.'/'.$design->thumbnail)}}" style="width:100%;">
                                    @endif
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="default-badge">
                                    ...
                                    @else
                                        <a href="javascript:;" onclick="updateDefault({{$design->id}})" class="text-dark mr-25 btn btn-sm btn-warning">
                                            Set Default
                                        </a>
                                    @endif
                                </div>
                            ...
                            <td class="text-center">
                                ...
                                <a href="javascript:;" onclick="designModal(true, {{$design->id}}, '{{$design->title}}','{{$design->product_id}}', {{$design->status_id}},'{{@$rgb_color[0]}}','{{@$rgb_color[1]}}','{{@$rgb_color[2]}}','{{$design->hex_color}}', '{{$design->thumbnail}}', '{{$design->design_type}}','{{$design->additional_thumbs}}','{{$design->layer}}')" data-toggle="tooltip" title="Edit Design" class="mr-25 table-icon-btn"> <i class="ft-edit"></i> </a>
                                <a href="javascript:;" onclick="deleteSwal({{$design->id}})" data-toggle="tooltip" title="Delete Design" class="table-icon-btn"> <i class="ft-trash-2"></i> </a>
                            </td>
                        </tr>
                    @endforeach
                @else
                                ...
                                <div class="col-sm-12 col-12 design-option brick-imgs" style="width:200px;display: none;">
                                    <div class="col" style="padding-left: 2px;">
                                        ...
                                        <div class="mr-2 upload-brick-1" style="display: none">
                                            <figure class="position-relative w-150 mb-0">
                                                <img src="{{asset('media/placeholder.jpg')}}" class="img-thumbnail brk-prev preview-b1">
...
@push('scripts')
    ...
    <script>
        ...
        const date = new Date();
        let thumbnail = null,view1 = null, isChange = null;
        ...
        function designModal(...values){
            ...
            form.reset();
            $(".brick-imgs, .texture-options, .library-check").hide();
            b = 1
            if(values[0] == true){
                ...
                if(values[9]!="" && values[9]!="null"){
                    ...
                    modal.find('#thumbnailImage').prev().attr('src', `{{asset('media/placeholder.jpg')}}`);
                }

                if(values[12]!="" && values[12]!="null"){
                    modal.find('#imageView1').prev().attr('src', `${path}/${values[12]}`);
                }else{
                    modal.find('#imageView1').prev().attr('src', `{{asset('media/placeholder.jpg')}}`);
                }
                ...
                if(values[11]){
                    ...
                    if(brickData.brick1){
                        ...
                        $(".upload-brick-1").hide();
                        $(".preview-b1").attr("src",`{{asset('media/placeholder.jpg')}}`);
                    }
                    if(brickData.brick2){
                        $(".add-brick").hide();
                        addBrickImg();
                ...
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        <span class="badge ${statusClass} text-uppercase">${status}</span>
                            ...
                            <td class="text-center">
                                <a href="javascript:;" onclick="designModal(true, ${response.id}, '${response.title}','${response.product_id}', ${response.status_id}, '${colorR}', '${colorG}', '${colorB}','${response.hex_color}','${response.thumbnail}','${response.design_type}','${response.additional_thumbs}','${response.layer}')" data-toggle="tooltip" title="Edit Design" class="table-icon-btn mr-25"> <i class="ft-edit"></i> </a>
        ...
        function deleteDesign(id){
            ...
            $.ajax({
                type: 'delete',
                url: '{{url("/api/delete-home-design")}}',
                data: {design_id: id },
                success: function(){
                    $(`#row${id}`).remove();
                    status = true;
        ...
        function updateDefault(designId){
            $.ajax({
                type: 'put',
                url: '{{url("/api/update-home-default")}}',
        ...
        $(document).ready(function (){
            ...
            $("input[name=design_type]").change(function (){
                ...
                if(design_type=="1"){
                    ...
                    $(".thumbnail").html("Thumbnail/Texture pattern");
                }
            });
            ...
            $(document).ready(function (){
                var category = "{{$design_type_slug}}".split("_");
                var formData = new FormData();
                formData.append('type', category[0]);
                $.ajax({
                    type: 'post',
                    url: "{{url("api/get-colors")}}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(response){
