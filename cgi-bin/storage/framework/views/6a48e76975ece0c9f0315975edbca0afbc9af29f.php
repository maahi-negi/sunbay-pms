								...
								<div class="panel panel-info">
									<div class="panel-body">
										...
										<form action="javascript:void(0)" id="signInForm" method="post">
											...
											<input type="hidden" id="btn_clicked" value="<?php if(auth()->guard()->check()): ?><?php echo e('finish'); ?><?php else: ?><?php echo e('login'); ?><?php endif; ?>">
                                            <?php echo csrf_field(); ?>
											<div id="lerr-msg"></div>
...
<script type="text/javascript">
    ...
    var app_base_url="<?php echo e(url('/')); ?>";
    // Sign up Form Action
    $('#signupForm').submit(function() {
        $.ajax({
            ...
            type : "POST",
            data : {'_token': "<?php echo e(csrf_token()); ?>",'name':$('#nname').val(),'email':$('#nemail').val(),'mobile':$('#nmobile').val(),'password':$('#npassword').val()},
            beforeSend  : function () {
                $("#preloader").show();
            },
            complete: function (response) {
    ...
    $('#signInForm').submit(function() {
       // $('#logindiv').html('<img src="'+app_base_url+'/images/spinner.gif">');
        $.ajax({
            ...
            type : "POST",
           // data : {'_token': "<?php echo e(csrf_token()); ?>",'email':$('#lemail').val(),'password':$('#lpassword').val()},
            data: $(this).serialize(),
            beforeSend  : function () {
                $("#preloader").show();
            },
            complete: function (response) {
                if(response.responseJSON.success == true) {
                  if($('#btn_clicked').val() == 'login'){
                   // window.location.reload();
                      window.location.href = "<?php echo e(route('homes.exterior')); ?>";
                  }else{
                    //final_finish();
                    //window.location.reload();
                      window.location.href = "<?php echo e(route('homes.exterior')); ?>";
    ...
    $('#forgotPasswordPopForm').submit(function() {
        ...
        $.ajax({
            ...
            type : "POST",
            data : {'_token': "<?php echo e(csrf_token()); ?>",'email':$('#femail').val()},
            beforeSend  : function () {
                //$("#preloader").show()
            },
            complete: function (response) {
    ...
    $('#verifyCode').click(function() {
        $.ajax({
            ...
            type : "POST",
            data : {'_token': "<?php echo e(csrf_token()); ?>",'email':$('#femail').val(), 'vcode':$('#fvcode').val()},
            beforeSend  : function () {
                //$("#preloader").show();
            },
            complete: function (response) {
    ...
    $('#cpPassBtn').click(function() {
        ...
        $.ajax({
            ...
            data : {'_token': "<?php echo e(csrf_token()); ?>",'email':$('#femail').val(), 'vcode':$('#fvcode').val(), 'passcode': $('#fpassword').val()},
...
<?php /**PATH /home/xhome/public_html/ule-uat.xhome360.com/cgi-bin/resources/views/elements/home-login.blade.php ENDPATH**/ ?>